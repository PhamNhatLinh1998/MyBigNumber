...
public class MyBigNumber {
	...
	public String sum(final String s1, final String s2) {
		...
		// Kiểm tra số âm
		if (s1.charAt(0) == '-') {
			ireceiver.send("Không hổ trợ số âm: " + s1);
			throw new NumberFormatException("Không hổ trợ số âm: " + s1);
		}

		if (s2.charAt(0) == '-') {
			ireceiver.send("Không hổ trợ số âm: " + s2);
		...
		for (int i = 0; i < len1; i++) {
			...
			if (matcher1.find()) {
				this.ireceiver.send("Vị trí " + (matcher1.start() + 1) + "trong chuổi " + s1 + " không phải là số");
				throw new NumberFormatException((matcher1.start() + 1) + "");
			}
		}
		...
		for (int i = 0; i < len2; i++) {
			...
			if (matcher2.find()) {
				this.ireceiver.send("Vị trí " + (matcher2.start() + 1) + "trong chuổi " + s2 + " không phải là số");
				throw new NumberFormatException((matcher2.start() + 1) + "");
			}
		}
